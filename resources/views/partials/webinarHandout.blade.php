<h5 class="card-title align-items-start flex-column mb-2">
    <span class="card-label font-weight-bolder text-dark">جزوه ها:</span>
</h5>

<table class="table table-borderless mb-10">
    <tbody>
        <!--begin::Item-->
        @forelse ($handouts as $handout)
        <tr>
            ...
            <td class="w-150px align-middle">                
                <div class="text-dark text-hover-primary mb-1 font-size-lg font-weight-bold">{{ $handout->name }}</div>
                <span class="text-muted">{{ $handout->description }}</span>
            </td>            <td class="font-weight-bolder font-size-lg text-dark-75 text-right align-middle" style="direction: ltr;">
                {{ round($handout->file->size / 1024 / 1024, 2) }} MB</td>
            <td class="font-weight-bolder font-size-lg w-25 text-right align-middle">
                <a href="{{ route('file.show', $handout->file->id) }}" class="btn btn-success">
        ...
        <tr>
            ...
            <td class="font-size-lg font-weight-bolder text-dark-75 w-100 align-middle">هیچ جزوه ای برای این دوره ثبت
                نشده است!!</td>
        </tr>
        @endforelse
    </tbody>
