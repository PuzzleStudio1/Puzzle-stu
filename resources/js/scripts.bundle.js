				...
				pagination: {
					items: {
						default: {
							...
							more: 'سایر صفحات',
							input: 'شماره صفحه',
							select: 'سایز صفحه را انتخاب کنید',
							all: 'همه',
						},
						info: 'نمایش {{start}} تا {{end}} از {{total}} مورد',
					},
				},
			},
		},
        ...
        setTimeout(function() {
            $.ajax({
                ...
                error: function(res) {
                    ...
                    _hideProgress();
                    KTUtil.addClass(_target, _resultClass);
                    KTUtil.setHTML(_resultWrapper, '<span class="font-weight-bold text-muted">ارتباط با سرور جستجو برقرار نشد. لطفا با پشتیبانی تماس بگیرید...</div>');
                    _showDropdown();
                    KTUtil.scrollUpdate(_resultWrapper);
                }
            });
